...
(() => {
  ...
  function isLoggedIn() { return localStorage.getItem("dashboard_demo_session") === "1"; }
  function logout() { localStorage.removeItem("dashboard_demo_session"); currentPage = null; render(); }
  function fullscreen() {
  ...
  function renderLogin() {
    ...
    root.innerHTML = `
      <section class="login-page">
        <form class="login-card" id="loginForm">
          ...
          <p class="subtitle">نسخه اولیه محلی برای بررسی ساختار صفحات و داده‌های اکسل</p>
          <div class="field">
          ...
          <div class="field">
            ...
            <input id="password" type="password" value="123456" autocomplete="current-password" required>
          </div>
          <button class="primary-btn" type="submit">ورود آزمایشی به سامانه</button>
          <div class="demo-note">
            این ورود صرفاً برای نسخه اولیه است و امنیت واقعی ایجاد نمی‌کند. پس از تأیید ظاهر و محتوا،
            همین صفحه به Supabase Auth متصل می‌شود.
          </div>
        </form>
      </section>`;
    root.querySelector("#loginForm").addEventListener("submit", e => {
      ...
      localStorage.setItem("dashboard_demo_session", "1");
      document.body.innerHTML = `<div id="app"></div>`;
      render();
    });
  }
        ...
        <div class="shell">
          <header class="topbar">
            ...
            <div class="top-actions">
              ...
              <button class="danger-btn" data-action="logout">خروج</button>
            </div>
          </header>
          <main class="main">
            <div class="notice">نسخه آزمایشی محلی است. داده‌ها از فایل اکسل نمونه تولید شده‌اند و برای انتشار اینترنتی با اطلاعات واقعی باید ابتدا لایه امنیتی فعال شود.</div>
            ${content}
          </main>
          <footer class="footer">${esc(data.settings.version || "")} — فایل داده قابل ویرایش: excel-data/dashboard-master.xlsx</footer>
        </div>
      </div>`;
  }
    ...
    const content = `
      <section class="hero">
        <div>
          ...
          <p>برای مشاهده جزئیات، یکی از صفحات اصلی را انتخاب کنید. هر صفحه شامل بخش‌های باز و بسته‌شونده، شاخص‌ها، نمودارها و جدول‌های مرتبط است.</p>
        </div>
      </section>
  ...
  function barChart(chart) {
    ...
    let svg = `<svg class="chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(chart.title)}">`;
    for(let i=0;i<=4;i++){
  ...
  function doughnutChart(chart) {
    ...
    let offset=0,svg=`<svg class="chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(chart.title)}">
  ...
  function renderAttachments(items=[]) {
    ...
    return `<div class="attach-box"><div class="box-title">فایل‌های ضمیمه</div>${items.map(a=>`
      ...
      <button class="soft-btn" title="در نسخه امن فعال می‌شود">📎 ${esc(a.file_name)}</button></div>`).join("")}</div>`;
  }

  function renderMap(layers=[]) {
    if(!layers.length) return "";
    return `<div class="map-box"><div class="box-title">نقشه تعاملی — جای‌نگهدار نسخه اولیه</div>
      <div class="map-placeholder"><div><div style="font-size:44px">🗺️</div><strong>لایه‌های مکانی در مرحله بعد به GeoJSON متصل می‌شوند.</strong>
      <div class="muted" style="margin-top:8px">در نسخه امن می‌توان محدوده قطعات، کاربری‌ها و پروژه‌ها را روی نقشه نمایش داد.</div></div></div>
  ...
  function renderReport(page) {
    ...
    const content=`
      ...
      ${sections || `<div class="empty">برای این صفحه هنوز محتوایی تعریف نشده است.</div>`}`;
  ...
  function wireCommon(){
    app.querySelectorAll('[data-action="logout"]').forEach(b=>b.addEventListener("click",logout));
